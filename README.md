...
macOS Finder 工具：提交并推送 Git 更改、创建空白 `README.md`，并在主应用中管理设置、操作历史和运行日志。
...
- Git：输入多行提交信息，默认当地时间 `yyyy-MM-dd HH:mm`；校验仓库和上游后，暂存**整个仓库**的所有更改，提交并推送当前分支到配置的上游。无新更改时仍可推送已有提交。
- 不自动初始化仓库，不自动设置远端，不切换分支，不强制推送。推送失败会保留本地提交。
- README：原子创建空文件，已有文件、目录或符号链接均不覆盖。
...
Finder Sync 每个扩展提供一个工具栏按钮，因此应用包含 Git 和 README 两个扩展。菜单使用标准菜单项，由扩展的主类响应动作；Git 多行编辑器位于弹出的提交框中。Finder 不提供可靠的菜单内自定义文本编辑器支持。
...
在 macOS 15 上，可从“系统设置 → 通用 → 登录项与扩展 → 扩展 → 文件提供程序”分别启用 `HappaTools` 和 `HappaTools README`。启用扩展后，还要在 Finder 的“显示 → 自定工具栏”中加入两个按钮。完整磁盘访问权限只需要按需授予外层 `HappaTools.app`，不能把沙盒扩展当作主应用授权。
