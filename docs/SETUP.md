...
2. 在“系统设置 → 通用 → 登录项与扩展 → 扩展 → 文件提供程序”中分别启用 **HappaTools** 和 **HappaTools README**。主应用概览提供入口；较旧 macOS 版本的入口名称可能不同。
3. 在 Finder 的“显示 → 自定工具栏”中添加 Git 和 README 按钮。关闭某项功能后，菜单操作禁用；移除按钮仍需使用 Finder 的自定工具栏。
